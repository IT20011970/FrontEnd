...
const AdministrationMasterTab3 = (props: any) => {
  ...
  const onChangeHandledBy = (e: React.ChangeEvent<HTMLInputElement>) => {
    // setHandledByName(e.target.value);
  ...
  // Add Handled By Person
  function post(){
    if ( handledByName == ''){
      alert('Please Select a Handled By Person!');
    } else {
      ...
      fetch('http://localhost:3000/handled-by-controller/post',requestOptions)
      setOpenmsg(true)
      }
      ...
      <Box sx={{ flexGrow: 1 }}>
        <Grid container spacing={2} >
          ...
          <SelectInput
                ...
                sx={{ textAlign: 'left' }}
                //value={handledByName}
                //onChange={e => handleChange(e.target.value, setHandledByName)}
                onChange={(e) => setHandledByName(String(e.target.value)) }
            >
              {handledBy.map(( row:ManageUserDetails, i: number) => (
        ...
        >
          ...
          <DialogContent>
            <DialogContentText id="alert-dialog-description">
              Handled By Person Added Successfully !
            </DialogContentText>
          </DialogContent>
          <hr/>
          <DialogActions>
...
export default AdministrationMasterTab3;
